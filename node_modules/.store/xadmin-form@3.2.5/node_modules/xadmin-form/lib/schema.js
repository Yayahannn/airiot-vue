...
exports.converters = exports.convert = void 0;
...
var convert = (schema, options) => {
  ...
  return _xadmin.app.get('schema_converter').reduce((prve, converter) => {
    return converter(prve, schema, opts);
  }, opts.global && opts.global.formDefaults ? _lodash.default.cloneDeep(opts.global.formDefaults) : {});
};
...
(f, schema, options) => {
  ...
  f.key = path.join('.');
  f.name = f.key;
  f.label = schema.title || _lodash.default.startCase(fieldKey);
...
(f, schema, options) => {
  if (stripNullType(schema.type) === 'object') {
    ...
    var fields = keys.filter(key => opts.ignore.indexOf(key) === -1).map(key => {
      ...
      return props[key] !== undefined ? _lodash.default.merge(convert(props[key], opts), form[key] || {}) : form[key];
    });
